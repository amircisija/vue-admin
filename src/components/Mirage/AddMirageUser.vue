          ...
          <v-row>
            <v-col cols="12" sm="3" class="pr-0 border-right pt-0">
              <v-card-text>
                ...
                <v-file-input @change="onFileSelected" :rules="rules" accept="image/png, image/jpeg, image/bmp"
                  placeholder="Choose profile image" prepend-icon="mdi-camera" label="Profile image" v-model="image">
                </v-file-input>
              </v-card-text>
              <v-card-text>
                <h3 class="mb-5 overline">Contract files</h3>
                <div class="file__input--1">
                  <v-file-input accept=".pdf" label="Choose a contract"></v-file-input>
                </div>
              </v-card-text>
                  ...
                  <v-card-text class="mb-0 pb-0">
                    ...
                    <v-container>
                      <v-form @submit.prevent :disabled="loading" ref="form">
                        <v-row>
                          <v-col cols="12" sm="10">
                            <v-switch color="success" v-model="user.isActive" :label="`Active user?`"></v-switch>
                          </v-col>
                          <v-col class="my-auto">
                            <v-btn :loading="loading" :disabled="loading" color="success" @click="addUser(user)">Add
                              User</v-btn>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12" lg="6">
                            <v-menu v-model="menu1" :close-on-content-click="true" max-width="290">
                              <template v-slot:activator="{ on, attrs }">
                                <v-text-field :value="computedStartingDate" clearable label="Contract started" readonly
                                  v-bind="attrs" v-on="on" @click:clear="dateStarting = null"></v-text-field>
                              </template>
                          ...
                          <v-col cols="12" lg="6">
                            <v-menu v-model="menu2" :close-on-content-click="true" max-width="290">
                              <template v-slot:activator="{ on, attrs }">
                                <v-text-field :value="computedEndingDate" clearable label="Contract ends" readonly
                                  v-bind="attrs" v-on="on" @click:clear="dateEnding = null"></v-text-field>
                              </template>
                        ...
                        <v-row class="mb-5 rounded form__row">
                          ...
                          <v-col>
                            <v-menu ref="menu3" v-model="menu3" :close-on-content-click="false"
                              ...
                              <template v-slot:activator="{ on, attrs }">
                                <v-text-field :value="computedDate" label="Birthday date" prepend-icon="event" readonly
                                  v-bind="attrs" v-on="on"></v-text-field>
                              </template>
                              <v-date-picker ref="picker" :max="new Date().toISOString().substr(0, 10)"
                                v-model="user.dob.date" @change="menu3 = false"></v-date-picker>
                            </v-menu>
                          </v-col>
                          <v-col cols="12" sm="6">
                          ...
                          <v-col cols="12" sm="6">
                            <v-text-field :counter="20" v-model="user.name.first" label="First name" required>
                            </v-text-field>
                          </v-col>
                          <v-col cols="12" sm="6">
                            <v-text-field :counter="20" v-model="user.name.last" label="Last name" required>
                            </v-text-field>
                          </v-col>
                          <v-col cols="12" sm="6">
                            <v-text-field :counter="20" v-model="user.parentName" label="Parent name" required>
                            </v-text-field>
                          </v-col>
                          <v-col cols="12" sm="6">
                          ...
                          <v-col cols="12" sm="6">
                            <v-text-field :counter="20" v-model="user.bankId" label="Bank ID" required></v-text-field>
                          </v-col>
                        </v-row>
                        <v-row class="rounded form__row">
                          ...
                          <v-col cols="12" sm="6">
                            <v-text-field :counter="20" v-model="user.phone" label="Telefon" required></v-text-field>
                          </v-col>
                          <v-col cols="12" sm="6">
                            <v-text-field :counter="30" v-model="user.email" label="Email" required></v-text-field>
                          </v-col>
                          <v-col cols="12" sm="4">
                            <v-text-field :counter="50" v-model="user.location.street.name" label="Address" required>
                            </v-text-field>
                          </v-col>
                          <v-col cols="12" sm="4">
                            <v-text-field :counter="10" v-model="user.location.street.number" label="Address Number"
                              required></v-text-field>
                          </v-col>
                          <v-col cols="12" sm="4">
                            <v-text-field :counter="50" v-model="user.location.city" label="Town" required>
                            </v-text-field>
                          </v-col>
                          <v-col cols="12" sm="4">
                            <v-text-field :counter="50" v-model="user.location.postcode" label="Postcode" required>
                            </v-text-field>
                          </v-col>
                          <v-col cols="12" sm="4">
                            <v-text-field :counter="50" v-model="user.location.municipality" label="Municipality"
                              required></v-text-field>
                          </v-col>
                          <v-col cols="12" sm="4">
                            <v-text-field :counter="50" v-model="user.location.country" label="Country" required>
                            </v-text-field>
                          </v-col>
                          <v-col cols="12" sm="8">
                            <v-select :items="departments" v-model="user.department" label="Department"></v-select>
                          </v-col>
                          <v-col cols="12" sm="4">
                            <v-switch color="success" v-model="user.travelExpenses" :label="
                      ...
                      <v-snackbar v-model="snackbar" :timeout="timeout" absolute centered color="deep-purple accent-4"
                        elevation="24">
                        {{ text }}
